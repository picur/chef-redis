...
describe_recipe 'redis::source' do
  ...
  describe "packages" do
    it "installs the required package from source" do
      file("#{node['redis']['source']['prefix']}/bin/redis-server").must_exist.with(:mode, "644").with(:owner, "root").with(:group, "root")
    end
  end

  describe "configuration" do
    it "has a port specific configuration file" do
      file("/etc/redis/#{node['redis']['port']}.conf").must_exist.with(:mode, "644").with(:owner, "root").with(:group, "root")
    end
  end

  describe "service" do
    it "has a executable init.d script" do
      file("/etc/init.d/redis").must_exist.with(:mode, "755").with(:owner, "root").with(:group, "root")
    end
